              ...
              <Reveal key={cabin.id} delay={80 * index}>
                <article className="flex h-full flex-col overflow-hidden rounded-sm border border-zenova-ink/10 bg-card">
                  ...
                  <div className="flex flex-1 flex-col p-7">
                    {cabin.code ? (
                      <p className="mb-2 text-xs uppercase tracking-[0.24em] text-zenova-gold">
                        {cabin.code}
                      </p>
                    ) : null}
                    ...
                    <div className="mt-auto flex flex-wrap items-center gap-3">
                      <Button
                        asChild
                        className="btn-sheen rounded-none bg-zenova-gold px-5 text-xs font-semibold uppercase tracking-[0.18em] text-zenova-ink hover:bg-zenova-gold/90"
                      >
                      ...
                      <Link
                        to="/cabins/$cabinId"
                        params={{ cabinId: cabin.id }}
                        className="border-b border-zenova-gold pb-1 text-xs font-semibold uppercase tracking-[0.18em] text-card-foreground hover:text-zenova-gold"
                      >
                      ...
                      <Button
                        ...
                        className="rounded-none border-zenova-gold/50 text-xs font-semibold uppercase tracking-[0.18em] text-card-foreground hover:border-zenova-gold hover:bg-zenova-gold/10 hover:text-zenova-gold"
                      >
          ...
          <Reveal className="mt-16">
            ...
            <div className="hidden overflow-x-auto rounded-sm border border-zenova-ink/10 sm:block">
              <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                <thead>
                  <tr className="bg-zenova-ink/[0.04] text-xs uppercase tracking-[0.2em] text-zenova-stone/70">
            ...
            <div className="space-y-4 sm:hidden">
              {vipCabins.map((c) => (
                ...
                >
                  <p className="mb-1 text-xs uppercase tracking-[0.2em] text-zenova-stone/70">
                    {vi ? "Hạng phòng" : "Cabin type"}
                  </p>
      ...
      <DialogContent className="max-h-[90vh] max-w-md overflow-y-auto rounded-sm border-zenova-ink/10 bg-card p-0 duration-300 sm:max-w-lg">
        <div className="relative h-40 w-full overflow-hidden bg-zenova-ink/10 sm:h-48">
          ...
          <div className="absolute bottom-0 left-0 p-5">
            {shown.code ? (
              <p className="mb-1 text-xs uppercase tracking-[0.24em] text-zenova-gold">{shown.code}</p>
            ) : null}
        ...
        <div className="px-5 pb-6 pt-2">
            ...
            <dl className="animate-in fade-in slide-in-from-bottom-1 divide-y divide-zenova-ink/10 border-y border-zenova-ink/10 text-sm duration-300">
              {rows.map((r) => (
                <div key={r.label} className="flex items-baseline justify-between gap-4 py-3">
                  <dt className="text-xs uppercase tracking-[0.2em] text-zenova-stone/70">{r.label}</dt>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Button
              asChild
              disabled={loading}
              className="btn-sheen flex-1 rounded-none bg-zenova-gold text-xs font-semibold uppercase tracking-[0.18em] text-zenova-ink transition-opacity hover:bg-zenova-gold/90"
            >
              <a href={QUOTE_LINK} target="_blank" rel="noopener noreferrer">
                {vi ? "Liên hệ báo giá" : "Request a quote"}
              </a>
            ...
            <Button
              variant="outline"
              onClick={onClose}
              className="flex-1 rounded-none border-zenova-ink/20 text-xs font-semibold uppercase tracking-[0.18em] text-card-foreground hover:bg-zenova-ink/5"
            >
              {vi ? "Đóng" : "Close"}
            </Button>
          </div>
...
function Spec({ label, value, wide }: { label: string; value: string; wide?: boolean }) {
  return (
    <div className={wide ? "col-span-2 sm:col-span-1" : ""}>
      <dt className="text-xs uppercase tracking-[0.24em] text-zenova-stone/70">{label}</dt>
