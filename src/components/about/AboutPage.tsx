...
import exteriorImg from "@/assets/gallery/zenova-exterior-01.jpg";
import lobbyImg from "@/assets/gallery/zenova-public-lobby-01.jpg";
import diningImg from "@/assets/gallery/zenova-dining-panorama-01.jpg";
    ...
    <div className="min-h-screen bg-zenova-ivory">
      ...
      <main>
        <section className="relative h-[65vh] min-h-[420px] w-full overflow-hidden">
          <img
            src={exteriorImg}
            alt={vi ? "Du thuyền Zenova trên vịnh Hạ Long" : "Zenova Cruise on Ha Long Bay"}
          ...
          <div className="absolute inset-0 flex items-end">
            <div className="mx-auto w-full max-w-7xl px-6 pb-16 lg:px-8">
              <p className="eyebrow mb-5 text-zenova-gold">{t.about.label}</p>
              <h1 className="max-w-3xl text-4xl tracking-[0.02em] text-zenova-ivory sm:text-5xl lg:text-6xl">
                {t.about.title}
              </h1>
            </div>
          </div>
        ...
        <section className="relative py-28 lg:py-36">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
              <Reveal>
                <p className="mb-6 text-lg leading-relaxed text-zenova-stone/90">
                  {t.about.body}
                </p>
              </Reveal>
        ...
        <section className="bg-zenova-warm/20 py-28 lg:py-36">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <Reveal className="mb-14 max-w-2xl">
              <p className="eyebrow mb-6 text-zenova-gold">{t.about.values.label}</p>
              ...
              {values.map((v, idx) => {
                ...
                return (
                  <Reveal key={v.title} delay={120 * idx}>
                    <div className="h-full border-t border-zenova-gold/40 pt-6">
                      <Icon className="mb-4 h-6 w-6 text-zenova-gold" strokeWidth={1.25} />
                      <h3 className="mb-2 text-xl tracking-[0.02em] text-zenova-ink">{v.title}</h3>
                      <p className="text-sm leading-relaxed text-zenova-stone/80">{v.desc}</p>
                    </div>
                  </Reveal>
                );
              })}
            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
              <Reveal>
                <div className="grid gap-5">
                  <img
                    src={lobbyImg}
                    alt={vi ? "Sảnh đón Zenova" : "Zenova lobby"}
                    loading="lazy"
              ...
              <Reveal delay={160}>
                <p className="eyebrow mb-6 text-zenova-gold">{vi ? "TẦM NHÌN" : "VISION"}</p>
                <h2 className="mb-6 text-4xl tracking-[0.02em] text-zenova-ink sm:text-5xl">
                  {vi
                ...
                <p className="text-lg leading-relaxed text-zenova-stone/90">
                  {vi
                    ? "Zenova không chỉ là một chuyến du ngoạn. Chúng tôi kết hợp kiến trúc tinh tế, dịch vụ chu đáo và cảnh quan kỳ vĩ để tạo nên những kỷ niệm khó quên giữa lòng di sản thiên nhiên thế giới."
                    : "Zenova is more than a sightseeing trip. We blend refined architecture, thoughtful service and a spectacular landscape to create unforgettable memories in the heart of a UNESCO natural wonder."}
                </p>
              </Reveal>
            </div>
          </div>
