    ...
    <div className="min-h-screen bg-zenova-ivory">
      ...
      <main>
        <section className="relative h-[60vh] min-h-[400px] w-full overflow-hidden">
          <img
            src={offers[0]?.hero}
            alt={lang === "vi" ? "Ưu đãi Zenova Cruise" : "Zenova Cruise offers"}
          ...
          <div className="absolute inset-0 flex items-end">
            <div className="mx-auto w-full max-w-7xl px-6 pb-16 lg:px-8">
              <p className="eyebrow mb-5 text-zenova-gold">{t.offers.label}</p>
              <h1 className="max-w-3xl text-4xl tracking-[0.02em] text-zenova-ivory sm:text-5xl">
                {t.offers.title}
              </h1>
              <p className="mt-4 max-w-xl text-zenova-ivory/85">{t.offers.subtitle}</p>
            </div>
          </div>
        </section>
        ...
        <div className="mx-auto max-w-7xl space-y-16 px-6 py-24 lg:px-8 lg:py-32">
          ...
          <Reveal className="border-t border-zenova-ink/10 pt-16 text-center">
            ...
            <p className="mx-auto mb-8 max-w-xl text-zenova-stone/85">
              {lang === "vi"
                ? "Đội ngũ Zenova sẽ tư vấn gói phù hợp nhất với lịch trình và nhóm của bạn."
                : "The Zenova team will recommend the best package for your schedule and group."}
            </p>
            <Button
              asChild
            ...
            <ul className="mb-6 grid gap-2 sm:grid-cols-2">
              {highlights.map((h) => (
                <li key={h} className="flex items-start gap-2 text-sm text-zenova-stone/85">
                  <Check className="mt-0.5 h-4 w-4 shrink-0 text-zenova-gold" />
                  {h}
                </li>
              ))}
            </ul>

            <div className="mb-6 border-t border-zenova-ink/10 pt-5">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.22em] text-zenova-ink">
                {t.offers.terms}
              </p>
